...
import { Upload, FileText, AlertCircle, CheckCircle2, TrendingUp, TrendingDown, DollarSign } from 'lucide-react';
...
export default function DataAnalysis() {
  const [pdfData, setPdfData] = useState<FinancialData[]>([]);
  const [selectedYear, setSelectedYear] = useState(new Date().getFullYear());
  const [isAnalyzing, setIsAnalyzing] = useState(false);
  const [comparisonResults, setComparisonResults] = useState<ComparisonResult[]>([]);
  const [uploadError, setUploadError] = useState<string | null>(null);

  const { data: yearlyReport, isLoading: yearlyLoading } = useGetYearlyReport(selectedYear);
  ...
  const performComparison = () => {
    ...
    const backendTotalIncome = centsToEur(yearlyReport.totalOverview.totalIncome);
    const pdfTotalIncome = pdfData.reduce((sum, data) => sum + data.totalIncome, 0);
    const incomeDiff = pdfTotalIncome - backendTotalIncome;
    const incomePercentDiff = backendTotalIncome !== 0 ? (incomeDiff / backendTotalIncome) * 100 : 0;
    ...
    results.push({
      ...
      status: Math.abs(incomePercentDiff) < 0.01 ? 'match' : Math.abs(incomePercentDiff) < 1 ? 'minor' : 'major',
    });

    // Compare total expenses
    const backendTotalExpenses = centsToEur(yearlyReport.totalOverview.totalExpenses);
    const pdfTotalExpenses = pdfData.reduce((sum, data) => sum + data.totalExpenses, 0);
    const expensesDiff = pdfTotalExpenses - backendTotalExpenses;
    const expensesPercentDiff = backendTotalExpenses !== 0 ? (expensesDiff / backendTotalExpenses) * 100 : 0;
    ...
    results.push({
      ...
      status: Math.abs(expensesPercentDiff) < 0.01 ? 'match' : Math.abs(expensesPercentDiff) < 1 ? 'minor' : 'major',
    });
    ...
    const profitPercentDiff = backendProfit !== 0 ? (profitDiff / backendProfit) * 100 : 0;
    ...
    results.push({
      ...
      status: Math.abs(profitPercentDiff) < 0.01 ? 'match' : Math.abs(profitPercentDiff) < 1 ? 'minor' : 'major',
    });

    // Compare monthly data
    pdfData.forEach((pdfMonth) => {
      const monthIndex = parseInt(pdfMonth.month) - 1;
      if (monthIndex >= 0 && monthIndex < 12 && yearlyReport.monthlyOverviews[monthIndex]) {
        ...
        const incomePercentDiff = backendIncome !== 0 ? (incomeDiff / backendIncome) * 100 : 0;
        ...
        const expensesPercentDiff = backendExpenses !== 0 ? (expensesDiff / backendExpenses) * 100 : 0;
        ...
        results.push({
          ...
          percentageDiff: expensesPercentDiff,
          status:
            Math.abs(expensesPercentDiff) < 0.01 ? 'match' : Math.abs(expensesPercentDiff) < 1 ? 'minor' : 'major',
        });
      }
    });
  ...
  const generateReport = () => {
    ...
    report += `PREPORUKE:\n`;
    if (majorIssues.length > 0) {
      report += `1. Provjerite funkcije backend izračuna za kategorije s velikim razlikama\n`;
      ...
      report += `2. Provjerite da sve komponente koriste centralizirane funkcije konverzije\n`;
    } else {
  ...
  return (
    <div className="container py-8">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">Usporedba PDF podataka s backend izračunima</p>
      </div>
      ...
      <div className="grid gap-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Učitajte PDF izvještaj za usporedbu s podacima iz sustava. Sustav će automatski analizirati podatke i
              identificirati razlike.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="flex flex-col gap-4">
              <div className="flex items-center gap-4">
                ...
                {isAnalyzing && <span className="text-sm text-muted-foreground">Analiziram...</span>}
              </div>
              ...
              <Alert>
                ...
                <AlertDescription>
                  Možete izvesti podatke iz sustava koristeći opciju "Izvezi u CSV" ili "Izvezi u PDF" u zaglavlju, a
                  zatim ručno usporediti s vašim PDF dokumentom. Sustav koristi centralizirane funkcije konverzije
                  (centsToEur) i hrvatske lokalne postavke za formatiranje valute s točno 2 decimalna mjesta.
                </AlertDescription>
              </Alert>
            </div>
          </CardContent>
        ...
        {yearlyLoading ? (
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Ovi podaci su izračunati korištenjem backend funkcija s pravilnim granicama datuma i konverzijom valute
              </CardDescription>
            </CardHeader>
            <CardContent>
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {yearlyReport.monthlyOverviews.map((overview, index) => {
                        ...
                        return (
                          <TableRow key={index}>
                            <TableCell className="font-medium">{monthNames[index]}</TableCell>
                            <TableCell className="text-right text-green-600">
                              {formatCurrency(centsToEur(overview.totalIncome))}
                            </TableCell>
                            <TableCell className="text-right text-red-600">
                              {formatCurrency(centsToEur(overview.totalExpenses))}
                            </TableCell>
                            <TableCell
                              className={`text-right font-semibold ${
                                Number(overview.profit) >= 0 ? 'text-green-600' : 'text-red-600'
                              }`}
                            >
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>Detaljni pregled razlika između PDF-a i backend podataka</CardDescription>
                </div>
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className="text-right">PDF vrijednost</TableHead>
                      <TableHead className="text-right">Backend vrijednost</TableHead>
                  ...
                  <TableBody>
                    {comparisonResults.map((result, index) => (
                      <TableRow key={index}>
                        ...
                        <TableCell className="text-right">{formatCurrency(result.pdfValue)}</TableCell>
                        <TableCell className="text-right">{formatCurrency(result.backendValue)}</TableCell>
                        <TableCell
                          className={`text-right font-semibold ${
              ...
              <div className="mt-6 grid gap-4 md:grid-cols-3">
                <div className="rounded-lg border border-green-200 bg-green-50 p-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-semibold text-green-900">Podudaranja</span>
                  </div>
                ...
                <div className="rounded-lg border border-yellow-200 bg-yellow-50 p-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-semibold text-yellow-900">Male razlike</span>
                  </div>
                  <div className="mt-2 text-3xl font-bold text-yellow-600">
                    {comparisonResults.filter((r) => r.status === 'minor').length}
                  </div>
                </div>
                ...
                <div className="rounded-lg border border-red-200 bg-red-50 p-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-semibold text-red-900">Velike razlike</span>
                  </div>
                  <div className="mt-2 text-3xl font-bold text-red-600">
                    {comparisonResults.filter((r) => r.status === 'major').length}
                  </div>
                </div>
              </div>
            </CardContent>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Informacije o implementaciji i izračunima</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4 text-sm">
              <div>
                ...
                <p className="text-muted-foreground">
                  Svi iznosi se pohranjuju u backend-u kao centi (Int) i konvertiraju se u eure korištenjem
                  centralizirane funkcije <code className="rounded bg-muted px-1 py-0.5">centsToEur()</code> koja dijeli
                  s 100. Formatiranje koristi hrvatske lokalne postavke s točno 2 decimalna mjesta.
                </p>
              </div>
              ...
              <div>
                ...
                <p className="text-muted-foreground">
                  ...
                  <code className="rounded bg-muted px-1 py-0.5">getMonthStartTimestamp()</code> i{' '}
                  <code className="rounded bg-muted px-1 py-0.5">getMonthEndTimestamp()</code> koje izračunavaju točne
                  granice mjeseca usklađene s backend logikom.
                </p>
              </div>
              ...
              <div>
                ...
                <ul className="list-inside list-disc text-muted-foreground">
                  <li>
                    <code className="rounded bg-muted px-1 py-0.5">getMonthlyOverview()</code> - Izračunava mjesečni
                    pregled
                  </li>
                  <li>
                    <code className="rounded bg-muted px-1 py-0.5">getYearlyOverview()</code> - Izračunava godišnji
                    pregled
                  </li>
                  <li>
                    <code className="rounded bg-muted px-1 py-0.5">getMonthlyReport()</code> - Generira mjesečni
                    izvještaj
                  </li>
                  <li>
                    <code className="rounded bg-muted px-1 py-0.5">getPdfFinancialReportData()</code> - Dohvaća podatke
                    za PDF izvještaj
                  </li>
                </ul>
              </div>
              ...
              <div>
                ...
                <p className="text-muted-foreground">
                  Sve komponente (Dashboard, Reports, QuickIncome, Transactions, Exports) koriste iste funkcije
                  konverzije kako bi se osigurala dosljednost. Funkcija{' '}
                  <code className="rounded bg-muted px-1 py-0.5">formatCurrency()</code> koristi se samo za prikaz, ne
                  za izračune.
                </p>
              </div>
            </div>
