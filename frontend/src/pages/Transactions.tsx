...
import { Plus, Edit, Trash2, TrendingUp, TrendingDown, Calendar, Filter, X } from 'lucide-react';
import { useGetAllTransactions, useDeleteTransaction } from '@/hooks/useQueries';
import { formatCurrency } from '@/lib/utils';
import { Transaction, TransactionType, ExpenseCategory } from '@/backend';
...
export default function Transactions() {
  ...
  const sortedTransactions = filteredTransactions.sort((a, b) => Number(b.date) - Number(a.date));
  ...
  return (
    <div className="container py-4 sm:py-8 px-4">
      <div className="mb-6 sm:mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-2xl sm:text-3xl font-bold tracking-tight">Transakcije</h2>
          <p className="text-sm sm:text-base text-muted-foreground">Upravljanje prihodima i rashodima</p>
        </div>
        <div className="flex gap-2 flex-wrap">
          ...
          <Button onClick={handleAdd} size="default" className="flex-1 sm:flex-none">
        ...
        <Card className="mb-6">
          ...
          <CardContent>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Select value={filterType} onValueChange={setFilterType}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value={TransactionType.prihod}>Prihod</SelectItem>
                    <SelectItem value={TransactionType.rashod}>Rashod</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              ...
              {/* Category Filter */}
              <div className="space-y-2">
                <label className="text-sm font-medium">Kategorija rashoda</label>
                <Select 
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value={ExpenseCategory.namirnice}>Namirnice</SelectItem>
                    <SelectItem value={ExpenseCategory.pice}>Piće</SelectItem>
                    <SelectItem value={ExpenseCategory.place}>Plaće</SelectItem>
                    <SelectItem value={ExpenseCategory.rezije}>Režije</SelectItem>
                    <SelectItem value={ExpenseCategory.oprema}>Oprema</SelectItem>
                    <SelectItem value={ExpenseCategory.napojnica}>Napojnica</SelectItem>
                    <SelectItem value={ExpenseCategory.ostalo}>Ostalo</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </div>
        ...
        <CardContent>
          {isLoading ? (
                ...
                >
                  <div className="flex items-start sm:items-center gap-3 sm:gap-4 min-w-0 flex-1">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 mb-1">
                        <p className="font-medium text-sm sm:text-base">{transaction.description || 'Bez opisa'}</p>
                        <Badge 
                          variant={transaction.transactionType === TransactionType.prihod ? 'default' : 'destructive'}
                          className="text-xs"
                        >
                          {transaction.transactionType === TransactionType.prihod ? 'Prihod' : 'Rashod'}
                        </Badge>
                        {transaction.transactionType === TransactionType.rashod && transaction.expenseCategory && (
                      ...
                      <div className="flex items-center gap-1.5 text-xs sm:text-sm text-muted-foreground">
                        ...
                        {new Date(Number(transaction.date) / 1000000).toLocaleDateString('hr-HR')}
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center justify-between sm:justify-end gap-3 sm:gap-4">
                    ...
                    <div className="flex gap-2">
                      <Button size="icon" variant="outline" onClick={() => handleEdit(transaction)} className="h-8 w-8 sm:h-10 sm:w-10">
            ...
            <div className="py-8 sm:py-12 text-center text-muted-foreground">
              {hasActiveFilters ? (
                <>
                  <p className="text-sm sm:text-base">Nema transakcija koje odgovaraju filterima</p>
        ...
        <AlertDialogContent className="max-w-[90vw] sm:max-w-md">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-xs sm:text-sm">
              Jeste li sigurni da želite obrisati ovu transakciju? Ova radnja se ne može poništiti.
            </AlertDialogDescription>
          </AlertDialogHeader>
