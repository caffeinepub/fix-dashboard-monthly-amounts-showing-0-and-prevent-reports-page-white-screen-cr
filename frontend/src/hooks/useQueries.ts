...
import { Transaction, TransactionInput, MonthlyIncomeInput, UserProfile, ExpenseShare, Projection, SimulationResult, BusinessProfile, BenchmarkPeriod, PerformanceAnalysis } from '@/backend';
...
export function useGetExpenseShareByCategory(startDate: bigint, endDate: bigint) {
  const { actor, isFetching } = useActor();
...
export function useRunSimulation() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (!actor) throw new Error('Actor not initialized');
      return actor.runSimulation(incomeGrowthPercentage, expenseGrowthPercentage);
    },
    retry: 1,
    retryDelay: 1000,
  });
...
export function useGetBusinessPerformanceAnalysis(
  ...
  const benchmarkPeriod: BenchmarkPeriod | null = (() => {
    ...
    } else if (period === 'monthly' && month !== undefined && year !== undefined) {
      return {
        __kind__: 'monthly',
  ...
  return useQuery<PerformanceAnalysis>({
    ...
    queryFn: async () => {
      ...
      const result = await actor.getBusinessPerformanceAnalysis(benchmarkPeriod);
...
export function useUpdateTransaction() {
  ...
  return useMutation({
    mutationFn: async ({ id, input }: { id: bigint; input: TransactionInput }) => {
      if (isReadOnly) {
        throw new Error('Operacija nije dostupna u načinu samo za čitanje');
      }
      if (!actor) throw new Error('Actor not initialized');
