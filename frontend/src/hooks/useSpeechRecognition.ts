...
export function useSpeechRecognition(
  ...
  const isSupported = browserSupport.hasSpeechRecognition && browserSupport.isHttps;
      ...
      recognition.onerror = (event: SpeechRecognitionErrorEvent) => {
        ...
        switch (event.error) {
          ...
          case 'audio-capture':
            ...
            break;
          case 'not-allowed':
            errorMessage = 'Pristup mikrofonu je odbijen. Omogućite dozvole u pregledniku.';
            break;
          case 'network':
  ...
  const startListening = useCallback(() => {
    if (!isSupported) {
      const errorMsg = 'Prepoznavanje govora nije podržano. Koristite Chrome ili Brave preko HTTPS veze.';
