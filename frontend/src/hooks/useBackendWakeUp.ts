...
export function useBackendWakeUp() {
  ...
  const performWakeUp = useCallback(async () => {
    ...
    try {
      ...
      console.warn(`Backend wake-up failed (attempt ${consecutiveFailuresRef.current}):`, error);
      
      // Only show error after multiple consecutive failures (3+)
      if (consecutiveFailuresRef.current >= 3) {
        console.error('Backend appears to be unresponsive after multiple wake-up attempts');
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.visibilityState === 'visible' && actor) {
        ...
        if (timeSinceLastWakeUp > 120000) { // 2 minutes
          performWakeUp();
        }
      }
    };
