import { Menu, RefreshCw, User, FileDown, FileText, AlertCircle, Mic, FileCode } from 'lucide-react';
...
export default function Header({ currentPage, onNavigate }: HeaderProps) {
  ...
  const text =
    loginStatus === 'logging-in' ? 'Prijava...' : isAuthenticated ? 'Odjava' : 'Prijava';
  ...
  const handleExportDfinityReport = async () => {
    try {
  ...
  const handleExportDfinityAnalysis = async () => {
    try {
  ...
  const handleExportExternalSupport = async () => {
    try {
      ...
      toast.error('Greška pri generiranju vanjskog izvještaja za DFINITY Support');
    }
  };
      ...
      <div className="container flex h-16 items-center justify-between px-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <h1 className="text-base font-bold text-primary sm:text-xl">Financije restorana</h1>
            ...
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent>
                  ...
                  <p className="text-xs text-muted-foreground">Koristi ako podaci nisu ažurirani</p>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
          )}
          ...
          {isAuthenticated && (
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-56">
                ...
                <DropdownMenuItem onClick={handleExportCSV} disabled={exportDataMutation.isPending}>
                  <FileDown className="mr-2 h-4 w-4" />
                  Izvezi podatke (CSV)
                </DropdownMenuItem>
                <DropdownMenuSeparator />
                ...
                {isAuthenticated && (
                  <>
                    <div className="flex flex-col gap-2 border-t pt-4">
                      <h3 className="text-sm font-semibold text-muted-foreground">Alati</h3>
                      <Button
                        variant="outline"
                        onClick={resync}
                        disabled={isResyncing}
                    ...
                    <div className="flex flex-col gap-2 border-t pt-4">
                      <h3 className="text-sm font-semibold text-muted-foreground">Izvoz podataka</h3>
                      <Button
                        variant="outline"
                        onClick={handleExportJSON}
                        disabled={exportDataMutation.isPending}
                    ...
                    <div className="flex flex-col gap-2 border-t pt-4">
                      <h3 className="text-sm font-semibold text-muted-foreground">Informacije</h3>
                      <DeploymentInfoDialog />
                    </div>
                  </>
                )}
