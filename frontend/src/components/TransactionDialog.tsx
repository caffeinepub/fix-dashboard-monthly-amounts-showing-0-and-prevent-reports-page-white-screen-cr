...
import { Transaction, TransactionType, ExpenseCategory, PaymentMethod } from '@/backend';
...
export default function TransactionDialog({ open, onOpenChange, transaction }: TransactionDialogProps) {
  ...
  const [transactionType, setTransactionType] = useState<'prihod' | 'rashod'>('prihod');
  ...
  useEffect(() => {
    if (transaction) {
      setTransactionType(transaction.transactionType === TransactionType.prihod ? 'prihod' : 'rashod');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const amountInCents = eurToCents(parseFloat(amount));
    const dateTimestamp = BigInt(new Date(date).getTime() * 1000000);

    const input = {
      amount: amountInCents,
      transactionType: transactionType === 'prihod' ? TransactionType.prihod : TransactionType.rashod,
      date: dateTimestamp,
      description,
      expenseCategory: transactionType === 'rashod' && expenseCategory ? (expenseCategory as ExpenseCategory) : undefined,
      paymentMethod: transactionType === 'prihod' && paymentMethod ? (paymentMethod as PaymentMethod) : undefined,
    };
      ...
      <DialogContent className="sm:max-w-[500px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription className="text-sm">
            {transaction ? 'Izmijeni podatke o transakciji' : 'Dodaj novu transakciju'}
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="type" className="text-sm sm:text-base">Tip transakcije</Label>
            <Select
              value={transactionType}
              onValueChange={(value: 'prihod' | 'rashod') => setTransactionType(value)}
              disabled={isReadOnly}
            >
          ...
          <div className="space-y-2">
            <Label htmlFor="amount" className="text-sm sm:text-base">Iznos (EUR)</Label>
            <Input
              id="amount"
              type="number"
              step="0.01"
          ...
          <div className="space-y-2">
            <Label htmlFor="date" className="text-sm sm:text-base">Datum</Label>
            <Input
              id="date"
              type="date"
              value={date}
          ...
          {transactionType === 'rashod' && (
            <>
              <div className="space-y-2">
                <Label htmlFor="category" className="text-sm sm:text-base">Kategorija</Label>
                <Select
              ...
              <div className="space-y-2">
                <Label htmlFor="description" className="text-sm sm:text-base">Opis</Label>
                <Textarea
                  id="description"
                  value={description}
          ...
          {transactionType === 'prihod' && (
            <div className="space-y-2">
              <Label htmlFor="payment" className="text-sm sm:text-base">Način plaćanja</Label>
              <Select
                value={paymentMethod}
          ...
          <DialogFooter className="flex-col sm:flex-row gap-2 sm:gap-0">
            ...
            <Button
              type="submit"
              disabled={addMutation.isPending || updateMutation.isPending || isReadOnly}
              className="w-full sm:w-auto text-sm sm:text-base"
            >
              {addMutation.isPending || updateMutation.isPending ? 'Spremanje...' : 'Spremi'}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
