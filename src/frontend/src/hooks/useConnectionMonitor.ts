...
export function useConnectionMonitor() {
  ...
  const wakeUpBackendAndSyncRelayer =
      ...
      try {
        ...
        if (!backendWokenUpRef.current) {
          const timeoutPromise = new Promise<never>((_, reject) => {
            setTimeout(
              () => reject(new Error("BACKEND_WAKEUP_TIMEOUT")),
              BACKEND_WAKEUP_TIMEOUT,
            );
          });
        ...
        if (!relayerSyncedRef.current) {
          const relayerTimeoutPromise = new Promise<never>((_, reject) => {
            setTimeout(
              () => reject(new Error("RELAYER_SYNC_TIMEOUT")),
              RELAYER_SYNC_TIMEOUT,
            );
          });
  ...
  const handleConnectionRestored = useCallback(async () => {
    ...
    toast.success("Veza uspostavljena", {
      description:
        "Backend je ponovno aktivan, relayer sinkroniziran, svi podaci su osvježeni",
      duration: 4000,
    });
  ...
  const attemptRecovery = useCallback(async () => {
    ...
    toastIdRef.current = toast.loading(
      `Ponovno uspostavljanje veze... (${recoveryAttemptsRef.current}/${MAX_RECOVERY_ATTEMPTS})`,
      {
        ...
        description:
          "Buđenje backenda, sinkronizacija relayera i osvježavanje actor bindings...",
      },
    );
    ...
    if (isConnected) {
      ...
      const delay =
        RECOVERY_RETRY_DELAY * 1.5 ** (recoveryAttemptsRef.current - 1);
  ...
  const handleConnectionLost = useCallback(() => {
    ...
    toastIdRef.current = toast.loading(
      "Ponovno uspostavljanje veze s backendom...",
      {
        duration: Number.POSITIVE_INFINITY,
        description:
          "Buđenje backenda, sinkronizacija relayera, osvježavanje actor bindings i canister ID mapiranje...",
      },
    );
