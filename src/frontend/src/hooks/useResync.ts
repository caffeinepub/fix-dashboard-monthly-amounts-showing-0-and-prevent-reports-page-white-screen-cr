...
export function useResync() {
  ...
  const resync = useCallback(async () => {
    ...
    try {
      ...
      if (!actorQueryData) {
        throw new Error(
          "Actor nije dostupan nakon rekreacije. Provjerite je li backend canister ID ažuran u env.json datoteci.",
        );
      }
      ...
      try {
        ...
        if (error?.message === "Backend timeout") {
          throw new Error(
            "Backend ne odgovara - provjerite je li canister aktivan i dostupan",
          );
        }
        throw new Error(
          "Backend nije dostupan - provjerite canister ID i mrežnu vezu",
        );
      }
      await new Promise((resolve) => setTimeout(resolve, 400));
      ...
      if (queriesCount === 0) {
        console.warn(
          "No queries in cache after resync - this may indicate an issue",
        );
      }
      ...
      toast.error("Greška pri resinkronizaciji", {
        ...
        description:
          error?.message ||
          "Pokušajte ponovno ili osvježite stranicu (F5). Provjerite je li backend canister ID ažuran u env.json datoteci.",
        duration: 10000,
      });
    } finally {
