...
import { MONTH_NAMES, centsToEur } from "./utils";
...
export async function exportToJSON(
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    try {
      ...
      if (!data) {
        throw new Error(
          "Podaci nisu dostupni. Backend je vratio prazan odgovor.",
        );
      }
      ...
      if (!data.transactions || !Array.isArray(data.transactions)) {
        throw new Error(
          "Neispravna struktura podataka: transakcije nisu pronađene.",
        );
      }

      if (!data.monthlyIncomes || !Array.isArray(data.monthlyIncomes)) {
        throw new Error(
          "Neispravna struktura podataka: mjesečni prihodi nisu pronađeni.",
        );
      }
  ...
  console.error(
    "[JSON Export] Svi pokušaji nisu uspjeli. Zadnja greška:",
    errorMessage,
  );

  throw new Error(
    `Izvoz JSON datoteke nije uspio nakon 2 pokušaja.\n\nRazlog: ${errorMessage}\n\nMolimo pokušajte ponovno za nekoliko trenutaka. Ako problem i dalje postoji, kontaktirajte podršku.`,
  );
}
...
export async function exportToPDFByYear(
  ...
  getPdfData: (year: bigint | null) => Promise<{
    overview: { totalIncome: bigint; totalExpenses: bigint; profit: bigint };
    transactions: any[];
    monthlyIncomes: any[];
    expensesByCategory: any[];
    incomeByPaymentMethod: any[];
  }>,
) {
  // Fetch financial data using backend calculation - SINGLE SOURCE OF TRUTH
  const financialData = await getPdfData(
    selectedYear !== null ? BigInt(selectedYear) : null,
  );
  ...
  const periodText =
    selectedYear !== null ? `Year: ${selectedYear}` : "Period: All years";
        ...
        <tbody>
          ${
            financialData.incomeByPaymentMethod.length > 0
              ? financialData.incomeByPaymentMethod
                  .map(
                    (pm) => `
              <tr>
      ...
      <table>
        ...
        <tbody>
          ...
          `,
                  ...
                  .join("")
              : '<tr><td colspan="6" style="text-align: center; padding: 20px;">No transactions in this period</td></tr>'
          }
        </tbody>
      </table>
